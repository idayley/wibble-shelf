...
const MODELS = [
  ...
  // Live-verified against claude 2.1.257 the day it shipped, by the same
  // probe every id above went through: `claude --model claude-fable-5-1
  // -p "reply with the single word: ok"` returned `ok` with exit 0.
...
[
  { id: "", label: "Default" },
  { id: "inherit", label: "Same as the conversation" },
  ...MODELS.map((m) => ({ id: m.id, label: m.label }))
];
const AGENT_ENGINES = "Claude Code, Codex and OpenCode";
const TOOLS_ON_CODEX = "Claude Code and OpenCode hold the agent to this list. Codex can't, so there it's asked to keep to it.";
const MODEL_ELSEWHERE = "A model runs on its own engine only. On the others this agent uses the conversation's model.";
function pinsAModel(model) {
...
const TRAY_SPAN = 150;
const ROPE = 104;
function hangingIn(a2) {
  const out = /* @__PURE__ */ new Set();
  for (const link of a2.links ?? []) {
...
function oklchHex(L, C, h) {
  ...
  return `#${lin.map((v3) => {
    const c2 = Math.max(0, Math.min(1, v3));
    const g = c2 <= 31308e-7 ? 12.92 * c2 : 1.055 * c2 ** (1 / 2.4) - 0.055;
...
function hexOklch(hex) {
  ...
  if (!m) return null;
  const s2 = m[1].length === 3 ? [...m[1]].map((c2) => c2 + c2).join("") : m[1];
  const [r, g, b2] = [0, 2, 4].map((i) => {
    const v3 = parseInt(s2.slice(i, i + 2), 16) / 255;
    return v3 <= 0.04045 ? v3 / 12.92 : ((v3 + 0.055) / 1.055) ** 2.4;
  });
  const l = Math.cbrt(0.4122214708 * r + 0.5363325363 * g + 0.0514459929 * b2);
  const mm = Math.cbrt(0.2119034982 * r + 0.6806995451 * g + 0.1073969566 * b2);
  const ss = Math.cbrt(0.0883024619 * r + 0.2817188376 * g + 0.6299787005 * b2);
  const L = 0.2104542553 * l + 0.793617785 * mm - 0.0040720468 * ss;
  const A = 1.9779984951 * l - 2.428592205 * mm + 0.4505937099 * ss;
  const B = 0.0259040371 * l + 0.7827717662 * mm - 0.808765396 * ss;
...
const ACCENTS = {
  brick: muted(35, 0.6, 0.075),
  pumpkin: muted(62, 0.66),
  butter: muted(95, 0.8, 0.075),
  sage: muted(135),
  teal: muted(190),
  sky: muted(235),
  plum: muted(295),
  rose: muted(320, 0.74, 0.065),
  cream: muted(85, 0.88, 0.02),
  charcoal: muted(250, 0.42, 0.012)
};
const MATERIAL_WORDS = {};
for (const [mat, words] of Object.entries({
  wood: "wood wooden timber plank bamboo cork paper cardboard bark",
  metal: "metal metallic steel iron tin aluminium aluminum chrome brass copper silver gold bronze stone rock concrete",
  glass: "glass ice crystal water gem",
  rubber: "rubber plastic foam latex vinyl balloon clay",
  cloth: "cloth fabric felt wool cotton canvas rope leather textile silk yarn",
  food: "food fruit vegetable veg bread cake candy sweet cookie biscuit cheese carrot apple snack"
})) {
  for (const w of words.split(" ")) MATERIAL_WORDS[w] = mat;
}
function readMaterial(v3) {
  if (typeof v3 !== "string") return void 0;
  return MATERIAL_WORDS[v3.trim().toLowerCase()];
}
const ACCENT_WORDS = {};
for (const [accent, words] of Object.entries({
  brick: "brick red crimson scarlet maroon cherry ruby",
  pumpkin: "pumpkin orange carrot tangerine amber peach coral brown rust copper",
  butter: "butter yellow gold golden lemon mustard sand",
  sage: "sage green lime olive mint leaf moss",
  teal: "teal cyan turquoise aqua",
  sky: "sky blue navy azure indigo cobalt",
  plum: "plum purple violet lavender lilac magenta",
  rose: "rose pink fuchsia salmon",
  cream: "cream white ivory beige tan",
  charcoal: "charcoal black grey gray silver slate"
})) {
  for (const w of words.split(" ")) ACCENT_WORDS[w] = accent;
}
function readAccent(v3) {
  if (typeof v3 !== "string") return void 0;
  const word = v3.trim().toLowerCase();
  if (word in ACCENT_WORDS) return ACCENT_WORDS[word];
  const c2 = hexOklch(word);
  if (!c2) return void 0;
  if (c2.C < 0.035) return c2.L > 0.7 ? "cream" : "charcoal";
  let best = "brick";
  let gap = Infinity;
  for (const name of ACCENT_NAMES) {
    const a2 = ACCENTS[name];
    if (a2.C < 0.03) continue;
    const d2 = Math.abs((c2.h - a2.hue + 540) % 360 - 180);
    if (d2 < gap) {
      gap = d2;
      best = name;
    }
...
function readTone(v3) {
  if (typeof v3 !== "number" || !Number.isFinite(v3)) return void 0;
  return v3 <= -0.5 ? -1 : v3 >= 0.5 ? 1 : 0;
}
const NEAR = {
  wood: ["cloth", "food", "metal", "rubber", "glass"],
  metal: ["glass", "rubber", "wood", "cloth", "food"],
  glass: ["metal", "rubber", "wood", "cloth", "food"],
  rubber: ["cloth", "metal", "glass", "food", "wood"],
  cloth: ["rubber", "wood", "food", "metal", "glass"],
  food: ["wood", "cloth", "rubber", "metal", "glass"]
};
function nearestOf(mat, allowed) {
  return NEAR[mat].find((m) => allowed.includes(m)) ?? allowed[0] ?? mat;
}
const MAX_SEGS = 240;
const MAX_D = 6e3;
function readPath(d2) {
  if (typeof d2 !== "string" || !d2.trim() || d2.length > MAX_D) return null;
  const tokens = d2.match(/[a-zA-Z]|-?(?:\d+\.?\d*|\.\d+)(?:e[-+]?\d+)?/g) ?? [];
  const out = [];
  let i = 0;
  let cmd = "";
  let cur = [0, 0];
  ...
  const num2 = () => {
    const t = tokens[i];
    if (t === void 0 || /[a-zA-Z]/.test(t)) return null;
    i += 1;
    const v3 = Number(t);
    return Number.isFinite(v3) ? Math.max(-4e3, Math.min(4e3, v3)) : null;
  };
  const pt = (rel) => {
    const x2 = num2();
    const y = num2();
    if (x2 === null || y === null) return null;
    return rel ? [cur[0] + x2, cur[1] + y] : [x2, y];
  };
  while (i < tokens.length && out.length < MAX_SEGS) {
    ...
    out.push(seg);
    cur = seg.p[seg.p.length - 1];
    lastCtrl = ctrl;
    lastKind = seg.c;
  }
  return out.some((s2) => s2.c !== "M" && s2.c !== "Z") ? out : null;
}
function writePath(segs) {
  const f = (v3) => String(Math.round(v3 * 10) / 10);
  return segs.map((s2) => s2.c + s2.p.map(([x2, y]) => `${f(x2)} ${f(y)}`).join(" ")).join("");
}
function mapPath(d2, fn) {
  const segs = readPath(d2);
  if (!segs) return d2;
  return writePath(segs.map((s2) => s2.c === "Z" ? s2 : { c: s2.c, p: s2.p.map(([x2, y]) => fn(x2, y)) }));
}
function pathBounds(d2) {
  const segs = readPath(d2);
  if (!segs) return null;
  let l = Infinity;
  let t = Infinity;
  ...
  for (const s2 of segs)
    for (const [x2, y] of s2.p) {
      l = Math.min(l, x2);
      r = Math.max(r, x2);
      t = Math.min(t, y);
      b2 = Math.max(b2, y);
    }
  return l === Infinity ? null : { x: l, y: t, w: r - l, h: b2 - t };
}
function readOutline(v3) {
  const segs = readPath(v3);
  return segs ? writePath(segs) : void 0;
}
...
function mod(num2, min, max) {
  ...
  if (max > min) {
    ...
    return num2 + (num2 < 0 ? max : min);
  } else {
    num2 = (num2 - max) % (min - max);
    return num2 + (num2 <= 0 ? min : max);
  }
}
function clamp$1(num2, min, max) {
  if (num2 < min) {
    return min;
  } else if (num2 > max) {
    return max;
  ...
  (function() {
    ...
    Solver2.prototype.solveIsland = function(step) {
      ...
      for (var i = 0; i < this.m_bodies.length; ++i) {
        ...
        copyVec2(body.c_position.c, c$1);
      ...
      for (var i = 0; i < this.m_bodies.length; ++i) {
        var body = this.m_bodies[i];
        copyVec2(c$1, body.c_position.c);
...
const PPM = 50;
Settings.linearSleepTolerance = 4 / PPM;
Settings.angularSleepTolerance = 0.4;
Settings.timeToSleep = 0.4;
const USE_VERBS = ["eat", "ride", "sit", "sleep", "push", "bounce", "climb", "carry"];
const SHAPES = ["bar", "disc", "ring", "box"];
const HOLDS = ["fixed", "pin", "loose"];
const SIZE_UNIT = 36;
function intoPart(p) {
  const c2 = Math.cos(-(p.angle ?? 0));
  const s2 = Math.sin(-(p.angle ?? 0));
  return (x2, y) => [(x2 - p.x) * c2 - (y - p.y) * s2, (x2 - p.x) * s2 + (y - p.y) * c2];
}
const USE_WORDS = {
  "sleep in": "sleep",
  sleepin: "sleep",
...
function readUse(v3) {
  const r = typeof v3 === "string" ? { verb: v3 } : v3 && typeof v3 === "object" ? v3 : null;
  if (!r || typeof r.verb !== "string") return void 0;
  const word = r.verb.trim().toLowerCase();
  const verb = USE_VERBS.includes(word) ? word : USE_WORDS[word];
  if (!verb) return void 0;
  const out = { verb };
  if (verb === "eat") out.bites = Math.round(clamp(num(r.bites, 4), 1, 8));
  if (verb === "ride" || verb === "sit" || verb === "sleep") out.seats = Math.round(clamp(num(r.seats, 1), 1, 4));
  return out;
}
function usePart(parts, verb, asked) {
  if (typeof asked === "number" && Number.isInteger(asked) && asked >= 0 && asked < parts.length) return asked;
  if (verb === "eat" || verb === "carry" || verb === "push") {
    const i = parts.findIndex((p) => (p.hold ?? "loose") === "loose");
    if (i >= 0) return i;
  }
  return 0;
}
...
function groundOf(s2) {
  const hung = hangingIn(s2);
  let lo = -1e9;
  for (const [i, p] of s2.parts.entries()) {
    if (p.hold === "pin" || hung.has(i)) continue;
    lo = Math.max(lo, reach(p).b);
  }
  return lo === -1e9 ? 0 : lo;
}
function heightOf(s2) {
  let lo = 1e9;
  let hi = -1e9;
...
function normalise(s2) {
  ...
  s2.w = Math.max(40, Math.round(hi - lo));
  return s2;
}
const MAX_PARTS = 14;
...
function num(v3, fallback) {
  return typeof v3 === "number" && Number.isFinite(v3) ? v3 : fallback;
}
function clamp(v3, lo, hi) {
  return Math.max(lo, Math.min(hi, v3));
}
function isShape(v3) {
  return typeof v3 === "string" && SHAPES.includes(v3);
}
function isHold(v3) {
  return typeof v3 === "string" && HOLDS.includes(v3);
}
const SIX = {
  slide: (p) => [
...
function readPart(p) {
  ...
  if (p.shape === "bar") {
    ...
    if (d2 < MIN_PLANK) {
      const k = d2 < 0.01 ? 0 : MIN_PLANK / d2;
      dx = d2 < 0.01 ? MIN_PLANK : dx * k;
      dy = d2 < 0.01 ? 0 : dy * k;
    }
...
function readLook(p, out) {
  const material = readMaterial(p.material);
  if (material) out.material = material;
  const tone = readTone(p.tone);
  if (tone) out.tone = tone;
}
function foldDecals(parts, loose) {
  ...
  if (!parts.length) {
    ...
    let r = -Infinity;
    let b2 = -Infinity;
    for (const d2 of loose) {
      const q = pathBounds(d2.outline);
      l = Math.min(l, q.x);
      t = Math.min(t, q.y);
      r = Math.max(r, q.x + q.w);
      b2 = Math.max(b2, q.y + q.h);
    }
    const w = Math.round(clamp(r - l, 4, MAX_W));
    const h = Math.round(clamp(b2 - t, 4, MAX_H));
    const [first, ...rest] = loose;
    const box = { shape: "box", hold: "loose", x: Math.round(l + w / 2), y: Math.round(t + h / 2), w, h };
    box.outline = mapPath(first.outline, intoPart(box));
    if (first.look.material) box.material = first.look.material;
    if (first.look.tone) box.tone = first.look.tone;
    parts.push(box);
    loose = rest;
  }
  for (const d2 of loose) {
    ...
    for (const p of parts) {
      const e3 = reach(p);
      const over = Math.max(0, Math.min(e3.r, q.x + q.w) - Math.max(e3.l, q.x)) * Math.max(0, Math.min(e3.b, q.y + q.h) - Math.max(e3.t, q.y));
      if (over > best) {
        best = over;
        host = p;
      }
    }
    host.decals = [...host.decals ?? [], { ...d2.look, outline: mapPath(d2.outline, intoPart(host)) }];
  }
}
function threeMaterials(parts) {
  const seen = [];
  const keep = (m) => {
    if (!m) return m;
    if (seen.includes(m)) return m;
    if (seen.length < 3) {
      seen.push(m);
      return m;
    }
    return nearestOf(m, seen);
  };
  for (const p of parts) {
    p.material = keep(p.material);
    for (const d2 of p.decals ?? []) d2.material = keep(d2.material);
  }
  for (const p of parts) {
    if (!p.material) delete p.material;
    for (const d2 of p.decals ?? []) if (!d2.material) delete d2.material;
  }
}
function readLinks(raw, parts) {
  ...
  for (const item of raw.slice(0, MAX_PARTS)) {
    ...
    const a2 = Math.round(num(l.a, -1));
    const b2 = Math.round(num(l.b, -1));
    if (a2 < 0 || b2 < 0 || a2 >= parts || b2 >= parts || a2 === b2) continue;
    const link = { a: a2, b: b2, kind };
    if (l.length !== void 0) link.length = Math.round(clamp(num(l.length, 100), 8, 600));
    if (kind === "pulley") {
      const over = Array.isArray(l.over) ? l.over.map((v3) => Math.round(num(v3, -1))) : [];
      if (over.length !== 2 || over.some((v3) => v3 < 0 || v3 >= parts)) continue;
...
function parseSpec(raw, fallbackName) {
  ...
  for (const item of rawParts.slice(0, MAX_PARTS)) {
    ...
    const outline = readOutline(p.outline);
    const part = readPart(p);
    if (part) {
      const use2 = readUse(p.use);
      if (use2) part.use = use2;
      if (outline) part.outline = stored ? outline : mapPath(outline, intoPart(part));
      if (Array.isArray(p.decals)) {
        const decals = [];
        for (const d2 of p.decals.slice(0, MAX_PARTS)) {
          if (!d2 || typeof d2 !== "object") continue;
          const dr = d2;
          const o = readOutline(dr.outline);
          if (!o) continue;
          const look = { outline: stored ? o : mapPath(o, intoPart(part)) };
          readLook(dr, look);
          decals.push(look);
        }
        if (decals.length) part.decals = decals;
      }
      parts.push(part);
    } else if (outline) {
      const look = { outline };
      readLook(p, look);
      loose.push({ outline, look });
    }
  }
  foldDecals(parts, loose.slice(0, MAX_PARTS));
  if (!parts.length) return null;
  threeMaterials(parts);
  const accent = readAccent(r.accent) ?? readAccent(rawParts[0]?.accent);
  for (const p of parts) {
    if (accent) p.accent = accent;
    else delete p.accent;
  }
  ...
  const use = readUse(r.use);
  if (use && !parts.some((p) => p.use)) {
    parts[usePart(parts, use.verb, r.use.part)].use = use;
  }
  for (const p of parts.filter((q) => q.use).slice(1)) delete p.use;
  const spec = normalise({ name, note, w: 0, parts, links, frame: "part" });
  if (accent) spec.accent = accent;
  if (typeof r.size === "number" && Number.isFinite(r.size)) {
    const tall = clamp(r.size, 0.25, 6) * SIZE_UNIT;
    const now2 = heightOf(spec);
    if (now2 > 0) scaleSpec(spec, tall / now2);
  }
...
function scaleSpec(s2, k) {
  for (const p of s2.parts) {
    ...
    if (p.thick !== void 0) p.thick = Math.round(clamp(p.thick * k, 3, 30));
    if (p.wall !== void 0) p.wall = Math.round(clamp(p.wall * k, 3, 24));
    const scaled = (x2, y) => [x2 * k, y * k];
    if (p.outline) p.outline = mapPath(p.outline, scaled);
    for (const d2 of p.decals ?? []) d2.outline = mapPath(d2.outline, scaled);
  }
  for (const l of s2.links ?? []) {
...
function parseFilter(value) {
  ...
  switch (v3.kind) {
    ...
    case "file":
      return FILE_KINDS.includes(v3.of) ? { kind: "file", of: v3.of } : null;
    case "agent": {
      const name = str$1(v3.name);
      return name ? { kind: "agent", name } : null;
    }
    case "data": {
      const field = str$1(v3.field);
      const op = DATA_OPS.includes(v3.op) ? v3.op : null;
      const raw = typeof v3.value === "number" ? String(v3.value) : typeof v3.value === "string" ? v3.value : null;
      return field && op && raw !== null ? { kind: "data", field, op, value: raw } : null;
    }
    default:
      return null;
  }
...
const ZONE_COLORS = Object.keys(ZONE_HUES);
...
function parsePages(value, zones) {
  ...
  for (const raw of value) {
    ...
    const p = raw;
    if (typeof p.zone !== "string" || !keys.has(p.zone)) continue;
    if (typeof p.file !== "string" || !p.file) continue;
    const num2 = (v3, d2) => typeof v3 === "number" && Number.isFinite(v3) ? v3 : d2;
    out.push({
      zone: p.zone,
      title: typeof p.title === "string" && p.title.trim() ? p.title.trim() : p.file,
      file: p.file,
...
function parseBundle(json) {
  ...
  try {
    ...
    return { ok: false, message: "Its kit file isn't valid JSON." };
  }
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    return { ok: false, message: "Its kit file isn't a kit." };
  }
  const v3 = raw;
  const items = [];
  const seen = /* @__PURE__ */ new Set();
  for (const entry of Array.isArray(v3.items) ? v3.items : []) {
    if (!entry || typeof entry !== "object") continue;
    const e3 = entry;
    if (typeof e3.kind !== "string" || typeof e3.id !== "string") continue;
    if (e3.kind === "bundle") return { ok: false, message: "It lists another kit inside it, and a kit can't hold a kit." };
    if (!ITEM_KINDS.includes(e3.kind)) continue;
    const key = `${e3.kind}/${e3.id}`;
    if (seen.has(key)) continue;
    seen.add(key);
    items.push({ kind: e3.kind, id: e3.id });
  }
  const zones = parseLayoutZones(v3.zones);
  if (items.length === 0 && zones.length === 0) {
    return { ok: false, message: "Its kit file lists nothing to add." };
  }
  return { ok: true, spec: { items, zones, pages: parsePages(v3.pages, zones) } };
}
function resolveBundle(spec, shelfItems) {
  const found = [];
  const missing = [];
  for (const want of spec.items) {
    const item = shelfItems.find((i) => i.kind === want.kind && i.id === want.id);
    if (item) found.push(item);
    else missing.push(`${want.kind} “${want.id}”`);
  }
...
function unquote(v3) {
  if (v3.length >= 2 && v3.startsWith('"') && v3.endsWith('"')) {
    return v3.slice(1, -1).replace(/\\"/g, '"').replace(/\\\\/g, "\\");
  }
  if (v3.length >= 2 && v3.startsWith("'") && v3.endsWith("'")) return v3.slice(1, -1).replace(/''/g, "'");
  return v3;
}
function agentMismatch(item, facts) {
  ...
  let differs = false;
  if (claims.tools) {
    const a2 = [...claims.tools].sort().join(",");
    const b2 = facts.tools === null ? "*" : [...facts.tools].sort().join(",");
    if (a2 !== b2) {
      differs = true;
...
function checkItem(item, files) {
  switch (item.kind) {
    ...
    case "bundle": {
      if (!files.bundleJson) return { ok: false, message: "Its repo has no kit file at that version." };
      const parsed = parseBundle(files.bundleJson);
      if (!parsed.ok) return { ok: false, message: parsed.message };
      return { ok: true, kind: "bundle", bundle: parsed.spec };
    }
  }
}
...
const KINDS = ["extension", "pin", "agent", "toy", "bundle"];
...
function readClaims(e3) {
  const tools = Array.isArray(e3.tools) ? e3.tools.filter((t) => typeof t === "string") : void 0;
  const model = str(e3.model)?.trim() || void 0;
  return tools || model ? { tools, model } : void 0;
}
function readArt(raw) {
  if (!raw || typeof raw !== "object") return void 0;
  const a2 = raw;
  const body = str(a2.body);
  const face = str(a2.face);
  const aspect = typeof a2.aspect === "number" && a2.aspect > 0.2 && a2.aspect < 5 ? a2.aspect : 0.85;
...
function parseEntry(raw) {
  ...
  const path = str(e3.path) ?? ".";
  if (!safePath(path)) {
    return { ok: false, reason: "It points outside its own repo." };
  }
  if ((kind === "agent" || kind === "toy" || kind === "bundle") && (path === "." || path === "")) {
    return { ok: false, reason: `It doesn't say which file in its repo is the ${kind === "bundle" ? "kit" : kind}.` };
  }
  const version = str(e3.version)?.trim();
  if (!version) {
    return { ok: false, reason: "It doesn't have a version number." };
  }
  const description = str(e3.description)?.trim() ?? "";
  const name = str(e3.name)?.trim().slice(0, 40) || nameFromId(id);
  return {
    ok: true,
    item: {
      kind,
      ...
      art: kind === "agent" ? readArt(e3.art) : void 0,
      history: readHistory(e3.history),
      claims: kind === "agent" ? readClaims(e3) : void 0
    }
  };
}
function parseIndex(raw) {
  ...
  obj.items.forEach((entry, i) => {
    ...
    const e3 = entry && typeof entry === "object" ? entry : {};
    const label = str(e3.name) || str(e3.id) || `Entry ${i + 1}`;
    if (!result.ok) {
...
function rawEntries(raw) {
  ...
  try {
    ...
    for (const e3 of parsed.items) {
      if (e3 && typeof e3 === "object") {
        const r = e3;
        out.set(`${String(r.kind)}/${String(r.id)}`, JSON.stringify(e3));
      }
    }
  } catch {
  }
...
function changes(base2, head) {
  ...
  if (!parsed.ok) return { changed: [], skipped: [], all: [], broken: parsed.message };
  ...
  return { changed, skipped: parsed.index.skipped, all: parsed.index.items };
}
const TOY_FIELDS = [
  "dx",
  "dy",
...
function toyProblems(raw, fallbackName) {
  ...
  const r = raw;
  const input2 = (r.parts ?? []).filter((p) => !!p && typeof p === "object");
  const problems = [];
  if (input2.some((p) => typeof p.k === "string")) {
    problems.push("It uses the old named parts (`k`). Describe it with shapes instead.");
    return problems;
  }
  if (input2.length !== spec.parts.length) {
    problems.push(`It lists ${input2.length} parts and ${spec.parts.length} can be built (unknown shapes are dropped, and the limit is 14).`);
    return problems;
  }
  input2.forEach((p, i) => {
...
function pngHasAlpha(bytes) {
  ...
  if (bytes.length < 33 || sig.some((b2, i) => bytes[i] !== b2)) return false;
...
function checkFiles(item, files, art, shelfItems = []) {
  const problems = [];
  const disclosure = [];
  ...
  if (!checked.ok) {
    ...
    disclosure.push(`Engine: ${AGENT_ENGINES}.`);
    disclosure.push(`Model: ${f.model || "not named (the engine picks)"}.${pinsAModel(f.model) ? ` ${MODEL_ELSEWHERE}` : ""}`);
    disclosure.push(`Tools: ${f.tools === null ? "every tool the engine has" : `${listOf(f.tools)}. ${TOOLS_ON_CODEX.replace(/\.$/, "")}`}.`);
  ...
  } else if (checked.kind === "bundle") {
    const { found, missing } = resolveBundle(checked.bundle, shelfItems);
    for (const m of missing) problems.push(`It lists ${m}, which this shelf doesn't have.`);
    disclosure.push(`Brings: ${found.map((i) => `${i.kind} \`${i.id}\``).join(", ") || "nothing but zones"}.`);
...
` };
  ...
  if (c2.skipped.length) {
    ...
    for (const s2 of c2.skipped) lines.push(`- **${s2.label}**: ${s2.reason}`);
    lines.push("");
  }
  ...
  for (const r of reports2) {
    ...
    if (r.disclosure.length) lines.push("What people will be told:", "", ...r.disclosure.map((d2) => `- ${d2}`), "");
  }
...
function kitPageFiles(json) {
  ...
  const pages = raw && typeof raw === "object" ? raw.pages : void 0;
  if (!Array.isArray(pages)) return [];
  return pages.flatMap((p) => p && typeof p === "object" && typeof p.file === "string" ? [p.file] : []);
}
function checkedRelPath(path) {
...
function readKitPage(root, dir, file) {
  const fail = (why) => ({ ok: false, why });
  if (!file.toLowerCase().endsWith(".html")) return fail(`${file} is not html`);
  const rel = checkedRelPath(file);
  if (rel === null) return fail(`${file} is outside the kit's folder`);
  const path = join(dir, rel);
  const missing = fail(`${file} isn't in the kit's repo`);
  let meta;
  try {
    meta = lstatSync(path);
  } catch {
  ...
  if (meta.isSymbolicLink()) return fail(`${file} is a link, not a file`);
  if (!meta.isFile()) return missing;
  let real;
  let realRoot;
  try {
  ...
  if (real !== realRoot && !real.startsWith(`${realRoot}/`)) return fail(`${file} is outside the kit's folder`);
  if (meta.size > MAX_PAGE_BYTES) return fail(`${file} is too large (over ${MAX_PAGE_BYTES / 1024} KB)`);
  let bytes;
  try {
  ...
  try {
    return { ok: true, html: new TextDecoder("utf-8", { fatal: true, ignoreBOM: true }).decode(bytes) };
  } catch {
    return fail(`${file} is not UTF-8 text`);
  }
}
function readKitPages(root, dir, json) {
  ...
  return { bundlePages, bundlePageErrors };
}
...
const png = (p) => {
  const b2 = readSmall(p, MAX_IMAGE);
  return b2 && b2.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])) ? b2 : null;
};
function checkOne(item, all) {
  ...
  try {
    ...
    if (item.kind === "extension" || item.kind === "pin") {
      ...
      files.bundleJson = text(target);
      if (files.bundleJson) Object.assign(files, readKitPages(dir, dirname(target), files.bundleJson));
    } else {
      files.toyJson = text(target);
    }
    const art = item.art ? { body: png(join(dir, item.art.body)), face: png(join(dir, item.art.face)) } : void 0;
    return checkFiles(item, files, art, all);
  } finally {
...
const reports = c.changed.map((item) => checkOne(item, c.all));
